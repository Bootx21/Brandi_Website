...
def query_blog():
    ...
    if posts_len < 2:
        ...
        posts = (Post.query.all())[::-1]
        posts_len = len(posts)
    print(posts[0].thumb, posts[0].picture, posts[0].body )
    hl = next(post for post in posts if post.id == int(posts[0].body))
    f1 = next(post for post in posts if post.id == int(posts[0].thumb))
    f2 = next(post for post in posts if post.id == int(posts[0].picture))
